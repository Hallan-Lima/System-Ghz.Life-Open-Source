...
const ModuleConfigCard: React.FC<ModuleConfigCardProps> = ({
  ...
  const handleIconClick = (e: React.MouseEvent) => {
    ...
    // Mapeamento de rotas atualizado para IDs numéricos (Baseado na tabela sys_module)
    // Ajuste os números conforme os IDs reais que estão na sua tabela do banco de dados
    const routes: Record<string, string> = {
      "1": "/tasks", // ID 1 = Produtividade
      "2": "/finance", // ID 2 = Financeiro
      "3": "/health", // ID 3 = Saúde
      "4": "/ia", // ID 4 = Inteligência Artificial
      "5": "/social", // ID 5 = Social
    };
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex items-center gap-4">
        ...
        >
          <div className="flex items-center gap-2">
            <h3
              className={`font-black text-lg truncate ${module.isEnabled ? "text-slate-800 dark:text-white" : "text-slate-500"}`}
            >
              {module.title}
            </h3>
            <span
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            ...
            >
              {module.isEnabled && (
                <i
                  className={`fas fa-check text-[10px] text-${module.color}-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}
                ></i>
              )}
            </div>
          </button>
          ...
          >
            <i
              className={`fas fa-chevron-down transition-transform duration-300 ${expanded ? "rotate-180" : ""}`}
            ></i>
          </button>
        </div>
      </div>
      ...
      <div
        className={`bg-slate-50 dark:bg-black/20 border-t border-slate-100 dark:border-slate-800 transition-all duration-500 ease-in-out ${
