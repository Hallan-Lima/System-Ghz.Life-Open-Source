...
const slides: OnboardingSlide[] = [
  {
    id: 1,
    category: 'Academia',
    title: 'Viva em alta frequência, aproveite cada momento.',
    description: 'Transforme sua rotina com monitoramento inteligente de treinos e saúde. O fluxo perfeito para sua evolução.',
  ...
  },
  {
    id: 2,
    category: 'Financeiro',
    title: 'Domine suas finanças com clareza absoluta.',
    description: 'Controle gastos, acompanhe investimentos e alcance suas metas financeiras com o auxílio do Ghz Assistant.',
  ...
  },
  {
    ...
    description: 'Lembretes de hidratação, sono e medicação. Uma vida saudável começa com pequenos hábitos integrados.',
    image: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?auto=format&fit=crop&q=80&w=1200',
    accentColor: 'from-indigo-600 to-purple-600'
  }
];

const categories = ['Academia', 'Financeiro', 'Saude', 'Agenda', 'Social', 'Alimentação'];
...
const FormInput: React.FC<FormInputProps> = ({ label, icon, as = 'input', children, ...props }) => {
  return (
        ...
        <div className="max-w-md w-full bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-2xl relative z-10 border border-white/10 ring-1 ring-black/5 animate-in fade-in zoom-in duration-300">
           ...
           <div className="mb-8 text-center relative">
             ...
             <div className="space-y-1 pt-2">
                <h3 className="text-2xl font-black text-slate-800 dark:text-white tracking-tighter">Login Hub</h3>
                <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">Seja bem-vindo</p>
             </div>
           </div>
           ...
           <form onSubmit={handleLogin} className="space-y-5">
              ...
              >
                {loading ? <i className="fas fa-spinner animate-spin"></i> : 'ACESSAR HUB'}
              </button>
            </form>
            ...
            <div className="mt-8 text-center space-y-6">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase">
                  <span className="bg-white dark:bg-slate-900 px-3 text-slate-400 font-bold tracking-widest">ou conecte via</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button className="flex items-center justify-center gap-2 py-3 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-bold text-xs hover:bg-slate-50 transition-all">
                  <i className="fab fa-apple text-slate-800 dark:text-white"></i> Apple
                </button>
              </div>

              <p className="text-xs text-slate-400 font-medium">
                Novo usuário? <Link to="/register" className="text-indigo-600 font-black hover:underline">Criar conta grátis</Link>
              </p>
            </div>
    ...
    <div className="relative h-screen w-full flex flex-col bg-slate-950 overflow-hidden font-sans">
      ...
      <div className="relative z-20 p-8 pt-12 flex justify-between items-start">
        <div className="w-12 h-12 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-2xl shadow-white/10 ring-4 ring-white/10">
          <div className="relative">
            <span className="text-indigo-600 font-black text-2xl italic tracking-tighter">H</span>
            <span className="absolute -bottom-1 -right-2 text-cyan-500 font-black text-xs">t</span>
          </div>
        </div>
        
        <div className="flex gap-2">
          <button onClick={prevSlide} className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white flex items-center justify-center active:scale-90 transition-all hover:bg-black/30">
            <i className="fas fa-chevron-left text-sm"></i>
          </button>
          <button onClick={nextSlide} className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white flex items-center justify-center active:scale-90 transition-all hover:bg-black/30">
            <i className="fas fa-chevron-right text-sm"></i>
          </button>
        </div>
      </div>

      <div className="relative z-20 mt-4 px-8 overflow-x-auto no-scrollbar flex gap-2">
        {categories.map((cat) => (
          <span 
            key={cat}
            className={`flex-shrink-0 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border backdrop-blur-md transition-all duration-500 ${
              cat === slide.category 
...
const MODULES = [
  { id: 'finance', title: 'Financeiro', icon: 'fas fa-wallet', color: 'text-emerald-500', desc: 'Controle gastos e metas.' },
  { id: 'health', title: 'Saúde', icon: 'fas fa-heartbeat', color: 'text-rose-500', desc: 'Monitore água, sono e treino.' },
  { id: 'productivity', title: 'Produtividade', icon: 'fas fa-list-check', color: 'text-indigo-500', desc: 'Tarefas e agenda inteligente.' },
  { id: 'journal', title: 'Diário', icon: 'fas fa-book-open', color: 'text-amber-500', desc: 'Registro de pensamentos.' },
];

const INTERESTS = [
  'Tecnologia', 'Investimentos', 'Musculação', 'Yoga', 'Leitura', 'Viagens', 'Culinária', 'Meditação', 'Artes', 'Games', 'Música', 'Estudos'
];
...
export const RegisterPage: React.FC = () => {
  ...
  const toggleModule = (id: string) => {
    setFormData(prev => {
      ...
      if (current.includes(id)) return { ...prev, selectedModules: current.filter(m => m !== id) };
  ...
  const toggleInterest = (interest: string) => {
   ...
   const toggleHealthGoal = (goal: string) => {
    setFormData(prev => {
      ...
      if (current.includes(goal)) return { ...prev, healthGoals: current.filter(g => g !== goal) };
  ...
  const renderProgressBar = () => (
    <div className="flex gap-2 mb-8">
      {[1, 2, 3].map(i => (
        <div key={i} className={`h-1.5 rounded-full flex-1 transition-all duration-500 ${i <= step ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-800'}`}></div>
      ))}
    </div>
  );
    ...
    <div className="flex flex-col min-h-screen bg-slate-950 p-6 relative overflow-x-hidden">
      ...
      <div className="max-w-md w-full mx-auto relative z-10 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6">
          <button onClick={handleBack} className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 text-white flex items-center justify-center active:scale-90 transition-all">
            <i className="fas fa-chevron-left text-sm"></i>
          </button>
          <span className="text-slate-400 font-bold text-xs uppercase tracking-widest">Passo {step} de 3</span>
        </div>
        ...
        {step === 1 && (
          <div className="animate-in fade-in slide-in-from-right duration-500 flex-1 flex flex-col">
             <div className="mb-6 space-y-2">
              <h2 className="text-3xl font-black text-white tracking-tighter">Quem é você?</h2>
              <p className="text-slate-400 text-sm">Precisamos de alguns dados essenciais para criar seu perfil no Hub.</p>
            </div>
        ...
        {step === 2 && (
           <div className="animate-in fade-in slide-in-from-right duration-500 flex-1 flex flex-col">
            <div className="mb-6 space-y-2">
              <h2 className="text-3xl font-black text-white tracking-tighter">Seu Foco</h2>
              <p className="text-slate-400 text-sm">Selecione o que é importante para você agora. Liberaremos os módulos conforme sua escolha.</p>
            </div>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-colors ${
                        isSelected ? 'bg-white/20 text-white' : `bg-slate-800 ${module.color}`
                      }`}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className={`text-xs font-medium mt-1 ${isSelected ? 'text-indigo-200' : 'text-slate-500'}`}>
                          {module.desc}
                        </p>
                      </div>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                        isSelected ? 'bg-white border-white' : 'border-slate-700'
                      }`}>
        ...
        {step === 3 && (
          <div className="animate-in fade-in slide-in-from-right duration-500 flex-1 flex flex-col pb-24">
            <div className="mb-6 space-y-2">
              <h2 className="text-3xl font-black text-white tracking-tighter">Personalização</h2>
              <p className="text-slate-400 text-sm">Vamos ajustar os detalhes finais para sua experiência.</p>
            </div>
            ...
            <div className="space-y-8">
              ...
              {formData.selectedModules.includes('productivity') && (
                <div className="bg-slate-900/50 rounded-3xl p-5 border border-slate-800 animate-in fade-in slide-in-from-bottom-4">
                   <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-white text-sm uppercase tracking-wide">Foco em Produtividade</h3>
                  </div>
                  <div className="space-y-3">
                     <button
                        onClick={() => updateProdConfig('enableGoals')}
                        className={`w-full p-4 rounded-2xl border flex items-center justify-between transition-all ${
                          formData.productivityConfig.enableGoals ? 'bg-indigo-500/10 border-indigo-500 text-indigo-400' : 'border-slate-800 text-slate-400 hover:bg-slate-800'
                        }`}
                     >
                        <div className="text-left">
                            <p className="font-black text-sm">Metas & Sonhos</p>
                            <p className="text-[10px] opacity-70">Planejamento a longo prazo e objetivos de vida.</p>
                        </div>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${formData.productivityConfig.enableGoals ? 'bg-indigo-500 border-indigo-500' : 'border-slate-600'}`}>
                            {formData.productivityConfig.enableGoals && <i className="fas fa-check text-[10px] text-white"></i>}
                        </div>
                     </button>
                     ...
                     <button
                        ...
                        className={`w-full p-4 rounded-2xl border flex items-center justify-between transition-all ${
                          formData.productivityConfig.enableShopping ? 'bg-indigo-500/10 border-indigo-500 text-indigo-400' : 'border-slate-800 text-slate-400 hover:bg-slate-800'
                        }`}
                     >
                        <div className="text-left">
                            <p className="font-black text-sm">Compras & Listas</p>
                            <p className="text-[10px] opacity-70">Listas rápidas, mercado e itens para adquirir.</p>
                        </div>
                         <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${formData.productivityConfig.enableShopping ? 'bg-indigo-500 border-indigo-500' : 'border-slate-600'}`}>
                            {formData.productivityConfig.enableShopping && <i className="fas fa-check text-[10px] text-white"></i>}
                        </div>
                     </button>
                  </div>
                </div>
              ...
              {formData.selectedModules.includes('finance') && (
                <div className="bg-slate-900/50 rounded-3xl p-5 border border-slate-800">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-white text-sm uppercase tracking-wide">Estilo Financeiro</h3>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                     <button
                        onClick={() => updateForm('financeMode', 'simple')}
                        className={`p-4 rounded-2xl border text-center transition-all ${
                          formData.financeMode === 'simple' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'border-slate-800 text-slate-400 hover:bg-slate-800'
                        }`}
                     >
                       <p className="font-black text-sm mb-1">Simples</p>
                       <p className="text-[10px]">Entradas, Saídas e Saldo Geral.</p>
                     </button>
                     <button
                        onClick={() => updateForm('financeMode', 'advanced')}
                        className={`p-4 rounded-2xl border text-center transition-all ${
                          formData.financeMode === 'advanced' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'border-slate-800 text-slate-400 hover:bg-slate-800'
                        }`}
                     >
                       <p className="font-black text-sm mb-1">Avançado</p>
                       <p className="text-[10px]">Categorias, cartões, metas e tags.</p>
                     </button>
                  </div>
                </div>
              )}
              ...
              {formData.selectedModules.includes('health') && (
                <div className="bg-slate-900/50 rounded-3xl p-5 border border-slate-800">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-white text-sm uppercase tracking-wide">Foco na Saúde</h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {['Hidratação', 'Medicamentos', 'Treinos', 'Sono', 'Calorias'].map(goal => (
                      <button
                        key={goal}
              ...
              {/* General Interests */}
              <div>
                <h3 className="font-bold text-white text-sm uppercase tracking-wide mb-4 pl-1">Interesses Gerais</h3>
                <div className="flex flex-wrap gap-2">
        ...
        <div className="mt-auto pt-6 bg-slate-950 sticky bottom-0 z-20 pb-4">
          ...
          >
            {loading ? <i className="fas fa-spinner animate-spin"></i> : (step === 3 ? 'FINALIZAR CRIAÇÃO' : 'PRÓXIMO')}
          </button>
          
          {step === 1 && (
             <p className="text-center mt-6 text-xs font-bold text-slate-400 uppercase tracking-widest">
              Já tem conta? <Link to="/login" className="text-indigo-500 hover:text-indigo-400">Entrar</Link>
            </p>
          )}
        </div>
